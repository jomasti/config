abbr -a pacupg sudo pacman -Su
abbr -a pacin sudo pacman -S
abbr -a pacins sudo pacman -Su
abbr -a pacre sudo pacman -R
abbr -a pacrem sudo pacman -Rns
abbr -a pacrep pacman -Si
abbr -a pacloc pacman -Qi
abbr -a paclocs pacman -Qs
abbr -a pacupl pacman -Qu
abbr -a pacupd sudo pacman -Sy
abbr -a paclist pacman -Ql
abbr -a pacorph pacman -Qdt
abbr -a pacmir sudo pacman -Syy
abbr -a pacexpl sudo pacman -D --asexplicit
abbr -a pacowns pacman -Qo
abbr -a pacclean sudo pcman -Sc
